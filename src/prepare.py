...
def _load_data(dataset=None):
    ...
    # If URL_DATA_PATH is set, always load from it (bypass cache)
    import os as _os2
    _env_path = _os2.environ.get("URL_DATA_PATH")
    if _env_path:
        print(f"Loading data from URL_DATA_PATH: {_env_path}")
        ...
        for item in data:
            ...
            label = None
            aidx = text.rfind("assistant\n")
            if aidx >= 0:
                ch = text[aidx + len("assistant\n"):aidx + len("assistant\n") + 1].strip().upper()
                label = 0 if ch == "A" else (1 if ch == "B" else None)
            if url and label is not None:
                urls.append(url)
                labels.append(label)
        n_a, n_b = labels.count(0), labels.count(1)
